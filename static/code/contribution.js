    ...
    class LexContribution extends qiki.LexClient {
        constructor(...args) {
            super(...args)
            var that = this;
            that.hack_console_verbose = false;
        ...
        get me() {
            return this.agent_from_idn(this.event_handlers.me_idn);
        }
        // is_me(user_idn) {
        //     return false;
        // }
            ...
            case that.idn_of.define:
                if (sbj === that.idn_of.lex) {
                    ...
                    if (parent === that.idn_of.category) {
                        return window.HackCategoryWord;
                    }
                }
                break;
            }
        ...
        cont_word_from_idn(cont_idn) {
            ...
            that.cats.loop(/** @param {CategoryWord} cat */ function (cat) {
                cont_word = cat.conts.get(cont_idn);
                if (cont_word !== null) {
                    return false;
                }
            });
        ...
        // cont_word_loop(callback) {
        //     var that = this;
        //     var return_value;
        //     that.cats.loop(/** @param {CategoryWord} cat */ function (cat) {
        //         cat.conts.loop(/** @param {ContributionWord} cont_word */ function (cont_word) {
        //             // var cont = Contribution.from_idn(cont_word.idn);
        //             return_value = callback(cont_word);
        ...
        assert_consistent() {
            ...
            that.cats.loop(/** @param {CategoryWord} cat */ function (cat) {
                cat.conts.loop(/** @param {ContributionWord} cont_word */ function (cont_word) {
                    console.assert(
                        cat === cont_word.cat,   // NOTE:  object instance comparison
    ...
    /*window.qiki ||= {};*/
    /*window.qiki.ContributionWord = */class ContributionWord extends qiki.Word {
        constructor(...args) {
            super(...args)
            var that = this;
        ...
        constructor(...args) {
            ...
            if (old_cont === null) {
                ...
                if (that.lex.is_authorized(that, old_cont.agent, "edit")) {
                    ...
                    if (that.lex.hack_console_verbose) {
    ...
    class CaptionWord extends qiki.Word {
        constructor(...args) {
            super(...args)
            var that = this;
            var cont = that.lex.cont_word_from_idn(that.obj.contribute);
            if (cont === null) {
        ...
        constructor(...args) {
            ...
            if (cont === null) {
                ...
                if (that.lex.is_authorized(that, old_cont_owner, action)) {
                    ...
                    cont.sbj = that.sbj;   // HACK:  Confusifying, e.g. because idn isn't changed.
